...
var isNativeApp = !!window.webkit?.messageHandlers?.tocData;
...
function buildMetaHeader(meta) {
	...
	var entries = Object.entries(meta).filter(
		(e) => skipKeys.indexOf(e[0]) === -1 && e[1],
	);
	...
	if (entries.length > 0) {
		...
		entries.forEach((entry) => {
			html +=
				"<tr><th>" +
				escapeHtml(entry[0].charAt(0).toUpperCase() + entry[0].slice(1)) +
				"</th>";
	...
	for (var i = 0; i < headings.length; i++) {
		...
		while (next) {
			if (
				...
				parseInt(next.tagName.charAt(1), 10) <= level
			)
				break;
			var sibling = next;
		...
		// Toggle handler
		var toggle = ((heading, content, dots) => (e) => {
...
function processFootnotes(container) {
	...
	footnotesSection.querySelectorAll("li.footnote-item").forEach((li) => {
		footnoteContents[li.id] = li.innerHTML.replace(_backrefRe, "").trim();
	});

	container.querySelectorAll("sup.footnote-ref").forEach((sup) => {
...
function initNativeScrollTracking() {
	...
	function reportActiveHeading() {
		...
		if (id !== _lastReportedId) {
			_lastReportedId = id;
			if (window.webkit?.messageHandlers?.scrollPosition) {
...
function expandHeading(heading) {
	...
	if (sib?.classList.contains("collapse-ellipsis")) {
		sib.style.display = "none";
		sib = sib.nextElementSibling;
	}
	if (sib?.classList.contains("collapsible-content")) {
...
window.scrollToHeading = (id) => {
	...
	while (parent && parent.id !== "content") {
		if (
			...
			if (prev?.classList.contains("collapse-ellipsis")) {
